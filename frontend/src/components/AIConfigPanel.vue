...
const config = ref<models.AIConfig>({
  provider: 'openai',
  apiKey: '',
  baseUrl: '',
  model: ''
...
async function saveConfig() {
  try {
    ...
    testResult.value = { success: false, message: '保存失败: ' + errorMessage }
  }
}
...
<template>
  <div class="ai-config-panel">
    <div class="panel-header">
      <h2>AI 配置</h2>
      <button @click="loadConfig" class="btn-refresh" :disabled="isLoading" title="重新加载">
        <span v-if="isLoading">⟳</span>
        <span v-else>⟳</span>
      </button>
    </div>

    <div v-if="isLoading" class="loading">加载配置中...</div>
    ...
    <div v-else class="config-form">
      ...
      <div class="form-group">
        ...
        <span class="help-text">
          默认: {{ providerPresets[config.provider].baseUrl }}
        </span>
      </div>
      ...
      <div class="form-group">
        ...
        />
        <span class="help-text">
          默认: {{ providerPresets[config.provider].model }}
        </span>
      </div>
      ...
      <div class="provider-info">
        ...
        <div v-if="config.provider === 'openai'" class="info-content">
          <p><strong>OpenAI</strong> 需要从 platform.openai.com 获取 API Key</p>
          <p>推荐模型: gpt-4, gpt-4-turbo, gpt-3.5-turbo</p>
        </div>
        <div v-else-if="config.provider === 'claude'" class="info-content">
          <p><strong>Anthropic Claude</strong> 需要从 console.anthropic.com 获取 API Key</p>
          <p>推荐模型: claude-3-opus-20240229, claude-3-sonnet-20240229</p>
        </div>
        <div v-else-if="config.provider === 'ollama'" class="info-content">
          <p><strong>Ollama</strong> 在本地运行，不需要 API Key</p>
          <p>请确保 Ollama 已在指定地址运行</p>
          <p>下载地址: ollama.ai</p>
        </div>
      </div>
    </div>
